...
def impute_missing_values(data, strategy='mean'):
    ...
    for col in data.columns:#check if there is missing value in each column
        if data[col].isnull().sum() > 0 and data[col].dtype in ['int64', 'float64']: #select columns that has integer or float data
            if strategy == 'mean':  
...
def normalize_data(data,method='minmax'):
    ...
    for col in num_cols:
        if method == "minmax":
            ...
            data[col] = (data[col] - minimum)/(maximum - minimum) #calculate using fomula
        elif method == "standard":
            mean = data[col].mean()
            stdv = data[col].std()
            data[col] = (data[col] - mean)/stdv
    return data
...
def remove_redundant_features(data, threshold=0.9):
    ...
    corr_matrix = num_data.corr().abs() #calculate correlation matrix of dataframe
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool)) # upper triangle of correlation matrix
    drop_cols = [column for column in upper.columns if any(upper[column] > threshold)] #find columns that has high correlation
    data.drop(drop_cols, axis=1, inplace=True) #drop highly-correlated columns
